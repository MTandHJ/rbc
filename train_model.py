...
def train(
    ...
    for epoch in range(start_epoch+1, epoches+1):
        ...
        for i, data in enumerate(trainloader):
            imgs, labels = data
            imgs = normalizer(imgs).to(device)
            labels = labels.to(device)

            outs, features = model(imgs)
            loss = loss_func(
                model=model, 
                outs=outs, 
                features=features,
...
if __name__ == "__main__":
    ...
    cfg, valider = load_cfg()

    # make path
    mkdirs(cfg.log_path, cfg.info_path)
    
    # set writter
    writter = SummaryWriter(log_dir=cfg.log_path, filename_suffix="train")

    # training
    train(valider=valider, path=cfg.info_path, **cfg)
